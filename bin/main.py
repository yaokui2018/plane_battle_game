...
def main():
    ...
    while running:
        ...
        if our_plane.active:
            ...
            # 绘制我方飞机的剩余血量
            pygame.draw.line(screen, color_black,
                             (our_plane.rect.left, our_plane.rect.top - 5),
                             (our_plane.rect.right, our_plane.rect.top - 5),
                             2)
            energy_remain = our_plane.energy / 100.0  # 假设最大血量是100
            if energy_remain > 0.2:  # 如果血量大约百分之二十则为绿色，否则为红色
                energy_color = color_green
            else:
                energy_color = color_red
            pygame.draw.line(screen, energy_color,
                             (our_plane.rect.left, our_plane.rect.top - 5),
                             (our_plane.rect.left + our_plane.rect.width * energy_remain, our_plane.rect.top - 5),
                             2)
        ...
        # 毁坏状态绘制爆炸的场面
            ...
            if not (delay % 3):
                ...
                if me_destroy_index == 0:
                    ...
                    pygame.display.flip()
                    pygame.time.wait(2000)  # 等待2秒钟，让玩家看到游戏结束的提示
        ...
        if our_plane.active:
            ...
            if enemies_down:
                if (delay % 3) == 0:
                    our_plane.energy -= 10  # 假设每次碰撞会减少10点血量
                    print("我方飞机血量剩余：", our_plane.energy)
                if our_plane.energy <= 0:  # 如果血量降到0或以下，飞机就不再存活
